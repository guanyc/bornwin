...
class StockNoteCallBack @Inject constructor(
    ...
    override fun onCreate(db: SupportSQLiteDatabase) {
        ...
        val currentLocale: Locale = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            applicationContext.resources.configuration.locales[0] // For Android N and above
        } else {
            applicationContext.resources.configuration.locale // For pre-Nougat Android versions
        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if(language.equals("en", ignoreCase = true)){
                var tas = listOf(//Buy low, break through, join forces
                    //Buy low, break through, join forces
                    TabEntity(title = "Break Through", tabType = TAB_TYPE.TAB_ACTION),
                    TabEntity(title = "Buy Low", tabType = TAB_TYPE.TAB_ACTION),
                    TabEntity(title = "Join Forces", tabType = TAB_TYPE.TAB_ACTION),

                    )

                var trs = listOf<TabEntity>(
                    TabEntity(title = "High Price", tabType = TAB_TYPE.TAB_REASON),
                    TabEntity(title = "New High", tabType = TAB_TYPE.TAB_REASON),
                    TabEntity(title = "Over Sold", tabType = TAB_TYPE.TAB_REASON),
                )
                ...
                var tas = listOf(
                    ...
                    TabEntity(title = "合力", tabType = TAB_TYPE.TAB_ACTION),
                    TabEntity(title = "低吸", tabType = TAB_TYPE.TAB_ACTION),
                    )

                var trs = listOf<TabEntity>(
                    TabEntity(title = "高标", tabType = TAB_TYPE.TAB_REASON),
                    TabEntity(title = "强度板", tabType = TAB_TYPE.TAB_REASON),
    ...
    object AppModule {
        ...
        fun provideMyBrainDataBase(
            ...
            return Room.databaseBuilder(
                ...
                StockNoteCallBack(provider, providerTarget, applicationContext)
